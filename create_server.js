  ...
  if(request.method == "GET") {
    switch(request.url) {
      ...
      default:
        ...
        console.log("Error: GET wrong url");
        response.end();
        break;
    }
    ...
    switch(request.url) {
      case "/": 
        ...
        request.on('end', function() {
          ...
          const origEmail = Aes.Ctr.decrypt(requestEmail, encryptionPassword, 256);
          let emailExists = check_credentials.email_check(origEmail);
      ...
      case "/SignInPassword":
        ...
        request.on("end", function(data) {
          ...
          while(data[i] != " ") {
            encryptedEmail += data[i];
            ++i;
          } //i is now at the space
          ++i;
          //continue until there's no more data
          for(i; i < data.length; ++i) {
            encryptedPassword += data[i];
          }

          const origEmail = Aes.Ctr.decrypt(encryptedEmail, encryptionPassword, 256);
          const origPassword = Aes.Ctr.decrypt(encryptedPassword, encryptionPassword, 256); //needs to be hashed later
          //writes back an answer
          response.writeHead(200, {'Content-Type': 'text/html'});
          //check if the encrypted email is in the database
          let loginCorrect = check_credentials.login_check(origEmail, origPassword);
          if(loginCorrect) {
            response.write("success");
          }else {
            response.write("failure");
...
//from https://gist.github.com/chrisveness/b28bd30b2b0c03806b0c
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -  */
